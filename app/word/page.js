...
import { 
  Download, UploadCloud, Settings, FileBox, 
...
export default function WordResizer() {
  const [file, setFile] = useState(null);
  const [processedFileUrl, setProcessedFileUrl] = useState(null);
  
  const [originalSize, setOriginalSize] = useState(0);
  const [targetSize, setTargetSize] = useState(500); // 500 KB Default
  ...
  const wordMemeQuotes = [
    "Trimming document media bytes... 🏋️‍♂️",
    "Faster than any cloud converter! ⚡",
    "Your document is now job portal ready! 🎯",
    "Grab a coffee, fixing your DOCX size in 1s... ☕",
    "100% Private: Processing stays inside your device! 🛡️",
    "Heavy DOCX Slimmer Engine Activated... 🚀",
  ];

  // 🎯 PRESETS FOR WORD DOCUMENTS
  const wordPresets = [
    { name: 'Govt Form Doc', size: 200, label: 'Max 200KB' },
    { name: 'Email Attachment', size: 500, label: 'Max 500KB' },
    { name: 'Portal Limit', size: 1000, label: 'Max 1MB' },
    { name: 'College Project', size: 2000, label: 'Max 2MB' },
  ];
  ...
  const onDrop = useCallback(async (acceptedFiles, rejectedFiles) => {
    if (rejectedFiles.length > 0) {
      setStatus('error');
      setErrorMessage('Invalid File! Please upload a valid Word (.docx) document.');
      return;
    }
    ...
    if (selectedFile) {
      ...
      if (!isValid) {
        setStatus('error');
        setErrorMessage('Security Alert: Corrupted or Fake DOCX file detected!');
        return;
      }
      ...
      if (sizeKB > 500) {
        setMode('decrease');
        setTargetSize(Math.min(500, Math.floor(sizeKB / 2)));
      } else {
        setMode('increase');
  ...
  // 🚀 REAL DEEP DOCX ZIP & MEDIA COMPRESSION ENGINE
  const processWord = async () => {
    if (!file) return;

    // Pick random meme quote
    const randomQuote = wordMemeQuotes[Math.floor(Math.random() * wordMemeQuotes.length)];
    ...
    try {
      ...
      if (mode === 'increase') {
        let docxBlob = await zipContent.generateAsync({ type: 'blob' });

        if (docxBlob.size < targetBytes) {
          const paddingNeeded = targetBytes - docxBlob.size;
          zipContent.comment = 'KBFixer_Padded_' + '0'.repeat(paddingNeeded);
          docxBlob = await zipContent.generateAsync({ 
            type: 'blob',
            compression: 'STORE'
          });
        }
        
        finishProcessing(docxBlob);

      } else {
        const mediaFiles = Object.keys(zipContent.files).filter((path) =>
          path.startsWith('word/media/') && /\.(png|jpg|jpeg)$/i.test(path)
        );
        ...
        for (const imagePath of mediaFiles) {
          try {
            ...
            console.log('Skipping unprocessable internal image:', imagePath);
          }
        }
      ...
      console.error(error);
      setStatus('error');
      setErrorMessage('Failed to process Word document. File might be corrupted or protected.');
    }
  };
  ...
  const shareFlex = () => {
    const text = `🔥 I just compressed my Word document from ${originalSize}KB to ${outputSize}KB in 0.1s using KBFixer by OnePersonAI! 100% Private & Free. Try it here: https://kbfixer.onepersonai.in`;
    ...
    <div className={`w-full max-w-5xl mx-auto px-3 sm:px-6 py-8 sm:py-12 flex flex-col items-center transition-all ${
      ...
      <div className="text-center max-w-3xl mb-8 sm:mb-12">
        <div className={`inline-flex items-center gap-2 px-3 py-1 border-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-4 hard-shadow ${
          ...
          <span>ZIP-Architecture DOCX Engine</span>
        </div>
        <h1 className="text-3xl sm:text-5xl md:text-6xl font-black uppercase tracking-tight mb-3 sm:mb-4">
          Word <span className={isBeastMode ? 'bg-white text-black px-2 sm:px-3' : 'bg-black text-white px-2 sm:px-3'}>Resizer</span>
        </h1>
        <p className="text-xs sm:text-base font-medium max-w-xl mx-auto border-l-4 border-current pl-3 text-left">
          Compress or increase DOCX document size in KB for email attachments, college projects, and portal submissions.
        </p>
      </div>
      ...
      <div className={`w-full border-3 sm:border-4 p-4 sm:p-8 md:p-10 hard-shadow transition-colors ${
        ...
        {!file && (
          ...
          >
            ...
            <h3 className="text-xl sm:text-2xl font-black uppercase mb-2">Drag & Drop Word Document Here</h3>
            ...
            <span className="inline-block border-2 border-current px-3 py-1 text-[10px] sm:text-xs font-black uppercase">
              DOCX Files Only (Deep Media Downscaling)
            </span>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
              ...
              <div className="space-y-4">
                <div className={`border-3 sm:border-4 p-4 sm:p-6 flex flex-col items-center justify-center h-48 sm:h-64 text-center ${
                  ...
                  <FileBox className="w-12 h-12 sm:w-20 sm:h-20 mb-3 sm:mb-4 stroke-[1.5]" />
              ...
              <div className="space-y-4 sm:space-y-6 flex flex-col justify-center">
                ...
                >
                  {status === 'processing' ? (
                    ...
                    <>
                      Resize Word Document <ArrowRight className="w-5 h-5" />
                    </>
                  )}
                </button>
              </div>
        ...
        {status === 'success' && processedFileUrl && (
          <div className="text-center py-6 sm:py-8 space-y-4 sm:space-y-6">
            ...
            <h2 className="text-2xl sm:text-3xl font-black uppercase">Word DOCX Processed Successfully!</h2>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              ...
              {targetSize <= 500 && (
                <span className="px-3 py-1 border-2 border-current text-xs font-black uppercase bg-black text-white">
                  🏆 Document Optimization Specialist
                </span>
              )}
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 pt-4">
              ...
              >
                <Download className="w-4 h-4 sm:w-5 sm:h-5" /> Download Fixed DOCX
              </a>
